...
# Build REAL evaluation dataset using parallel feature computation
#
# Final saved structure is unchanged:
#   series_id, series_name, x, xx, labels, feature columns...
#
# Design:
#   1) Build lightweight base_df sequentially
#   2) Pass only one lightweight row per worker
#   3) Workers return full final row with same structure as original script
# =====================================================================
...
for (period_i in periods) {
  ...
  for (window_mode_i in window_modes) {
    ...
    # ---------------------------------------------------------------
    # Step 1: Build lightweight base rows sequentially
    # This avoids exporting M4_period to each parallel worker.
    # ---------------------------------------------------------------
    ...
    build_base_row <- function(i) {
      ...
      if (length(x_full) < 1L) return(NULL)
      if (length(xx_full) < 1L) return(NULL)
    ...
    # ---------------------------------------------------------------
    # Step 2: Worker computes features and returns the exact final row
    # Structure matches original script output:
    #   series_id, series_name, x, xx, labels, feature columns...
    # ---------------------------------------------------------------
    ...
    compute_one_row <- function(row) {
      ...
      if (is.null(featrow)) return(NULL)
