...
# Apply standardisation to rolling windows
# =====================================================================
...
for (period_i in periods) {
  ...
  for (window_mode_i in window_modes) {
    ...
    if (!file.exists(windows_raw_i)) {
      cat("[04] Skip", period_i, window_mode_i, "- missing input:", windows_raw_i, "\n")
      next
    }
    ...
    if (RUN_PARALLEL) {
      ...
      cache_dir_transf <- file.path(
        data_dir,
        paste0("cache_transf_", TAG_i, "_", WINDOW_TAG_i)
      )
      ...
      rows <- run_step_parallel(
        ...
        step_name = paste0("transform_windows_", TAG_i, "_", WINDOW_TAG_i)
      )
      
    } else {
      
      rows <- lapply(seq_len(n_windows), transform_one_window)
    }
    ...
    saveRDS(all_windows_std, file = windows_std_i)
    cat("Saved transformed windows to:", windows_std_i, "\n")
