...
#
# Purpose:
#   Evaluate the Euclidean 1-nearest-neighbour baseline on real M4 series.
# Inputs:
#   Global configuration and labeled, split-index, and real-evaluation RDS files.
# Outputs:
#   One consolidated Euclidean evaluation RDS file per frequency.
# Run from:
...
# Pad or truncate numeric series to a common width.
...
for (period_i in periods) {
  ...
  for (window_mode_i in window_modes) {
    ...
    rm(
      df, split_index, split_use, train_idx, train_df,
