...
#
# Purpose:
#   Load R pipeline artifacts and prepare them for Python model runners.
# Inputs:
#   RDS datasets, split indices, model identifiers, horizons, and window modes.
# Outputs:
#   Model-ready feature, series, Mantis, and Chronos datasets.
# Used by:
#   The conventional-classifier, Chronos, and Mantis experiment runners.
# ==============================================================================
...
def _sanitize_feature_df(
    ...
    if bad_left.any():
        print(
            f"[WARN] {name}: non-finite values remained after sanitization; "
            "forcing to 0"
        )
...
def window_tag(window_mode: str) -> str:
    ...
    if value not in mapping:
        raise ValueError(
            "Invalid window_mode: {}".format(window_mode)
        )
...
def _normalise_list_column(
    col: pd.Series,
) -> List[np.ndarray]:
    return [
        _to_numpy_1d(value, dtype=float)
        for value in col.tolist()
    ]


def _normalise_labels_column(
    col: pd.Series,
) -> List[np.ndarray]:
    return [
        _to_numpy_1d(value, dtype=int)
        for value in col.tolist()
    ]


def _normalise_eval_df(
    df: pd.DataFrame,
) -> pd.DataFrame:
    out = df.copy()
...
def get_model_split(
    ...
    if str(model_id).strip() == "HIVECOTEV2":
        ...
        print(
            "[HIVECOTEV2] Training index first-half subset: "
...
def build_features_dataset(
    ...
    if missing_test:
        print(
            f"[WARN] Missing columns in X_test, dropping: {missing_test}"
        )

    if missing_real:
        print(
            f"[WARN] Missing columns in X_real, dropping: {missing_real}"
        )
    ...
    X_train = _sanitize_feature_df(
        X_train,
        "X_train",
    )
    X_test = _sanitize_feature_df(
        X_test,
        "X_test",
    )
    X_real = _sanitize_feature_df(
        X_real,
        "X_real",
    )
...
def build_series_dataset(
    ...
    all_lengths = (
        [len(value) for value in x_train]
        + [len(value) for value in x_test]
        + [len(value) for value in x_real]
    )
    target_len = int(max(all_lengths))

    padder = PaddingTransformer(
        pad_length=target_len
    )
    ...
    X_train_np3d = convert_to(
        X_train_pad,
        to_type="numpy3D",
        as_scitype="Panel",
    )
    X_test_np3d = convert_to(
        X_test_pad,
        to_type="numpy3D",
        as_scitype="Panel",
    )
    X_real_np3d = convert_to(
        X_real_pad,
        to_type="numpy3D",
        as_scitype="Panel",
    )
...
def get_model_data(
    ...
    all_windows_df = _normalise_eval_df(
        read_rds(paths.features_rds).copy()
    )
    split_index = read_rds(paths.split_index_rds)
    real_eval_df = _normalise_eval_df(
        read_rds(paths.real_eval_rds).copy()
    )

    split_use = get_model_split(
        split_index,
        model_id=model_id,
    )
    ...
    train_df = all_windows_df.iloc[train_idx].reset_index(
        drop=True
    )
    test_df = all_windows_df.iloc[test_idx].reset_index(
        drop=True
    )
    real_df = real_eval_df.reset_index(
        drop=True
    )
    ...
    y_train = np.asarray(
        [int(value[h0]) for value in train_df["labels"]],
        dtype=int,
    )
    y_test = np.asarray(
        [int(value[h0]) for value in test_df["labels"]],
        dtype=int,
    )
    y_real = np.asarray(
        [int(value[h0]) for value in real_df["labels"]],
        dtype=int,
    )
...
def get_mantis_data(
    ...
    X_train = [
        _to_numpy_1d(value, dtype=float)
        for value in train_df["x"].tolist()
    ]
    X_test = [
        _to_numpy_1d(value, dtype=float)
        for value in test_df["x"].tolist()
    ]
    X_real = [
        _to_numpy_1d(value, dtype=float)
        for value in real_df["x"].tolist()
    ]
    ...
    return {
        ...
        "y_real": y_real,
        "series_name_real": (
            real_df["series_name"].tolist()
            if "series_name" in real_df.columns
            else None
        ),
    }
...
def get_chronos_data(
    ...
    all_df = _normalise_eval_df(
        read_rds(paths.labeled_rds).copy()
    )
    real_df = _normalise_eval_df(
        read_rds(paths.real_eval_rds).copy()
    )

    for column in ["x", "labels"]:
        if column not in all_df.columns:
            raise ValueError(
                f"Missing column '{column}' in {paths.labeled_rds}"
            )

        if column not in real_df.columns:
            raise ValueError(
                f"Missing column '{column}' in {paths.real_eval_rds}"
            )
    ...
    if str(window_mode).strip().lower() == "full":
        ...
        test_idx = split_use["test"]
        test_df = all_df.iloc[test_idx].reset_index(
            drop=True
        )
    ...
    return {
        ...
        "y_test": [
            _to_numpy_1d(value, dtype=int)
            for value in test_df["labels"].tolist()
        ],
        "X_real": [
            _to_numpy_1d(value, dtype=float)
            for value in real_df["x"].tolist()
        ...
        "y_real": [
            _to_numpy_1d(value, dtype=int)
            for value in real_df["labels"].tolist()
        ],
        "series_name_test": (
            test_df["series_name"].tolist()
            if "series_name" in test_df.columns
            else None
        ),
        "series_name_real": (
            real_df["series_name"].tolist()
