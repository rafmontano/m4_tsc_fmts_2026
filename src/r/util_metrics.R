...
# util_metrics.R
#
# Purpose: Provide shared dataset, metric, and evaluation-object utilities.
# Inputs:  Model splits, predictions, M4 series, features, and configuration.
# Outputs: Evaluation summaries, detailed results, and saved evaluation objects.
# Run from: Repository root; sourced by R modelling and evaluation scripts.
# ==============================================================================

# Split helpers ---------------------------------------------------------------

# Use the uncapped split when a model-specific split is unavailable.
...
make_empty_eval_summary_row <- function(
  period_i,
  tag_i,
  eval_type,
  horizon_i,
...
compute_binary_eval <- function(
  ...
  f1_class <- function(p, r) {
    if (!is.finite(p) || !is.finite(r) || (p + r) == 0) {
      return(NA_real_)
    }
    2 * p * r / (p + r)
  }
...
get_predictor_cols_train_safe <- function(train_df, predictor_cols_base) {
  ...
  feature_vars <- sapply(
    feature_df_train,
    function(col) var(col, na.rm = TRUE)
  )
  constant_cols <- names(
    feature_vars[feature_vars == 0 | is.na(feature_vars)]
  )
...
build_real_eval_dataset <- function(
  M4_clean_all,
  period_i,
  feature_eng = "da",
  ...
  rows <- lapply(seq_along(M4_period), function(i) {
    ...
    tibble::tibble(
      ...
      labels = list(
        compute_label_vector(scaled$x_std, scaled$xx_std)
      )
    ) |>
...
store_real_eval_result <- function(
  containers,
  h,
  res_real,
  series_name = NULL
...
build_consolidated_eval_object <- function(
  containers,
  real_eval_df = NULL
) {
  out <- list(
...
save_consolidated_eval_object <- function(
  eval_obj,
  eval_rds_path,
  window_mode = "default"
) {
