...
#   for each frequency (counts + proportions) from REAL eval exports.
#
# Preferred input (per frequency):
#   data/export/real_eval_tsc_l{LABEL_ID}_{freq_tag}.csv
#
# Supported fallbacks:
#   data/export/real_eval_l{LABEL_ID}_{freq_tag}.csv
...
find_real_eval_file <- function(label_id, tag, export_dir = file.path("data", "export")) {
  ...
  if (!dir.exists(export_dir)) return(NA_character_)
...
map_class_label <- function(vals) {
  ...
  if (length(u) <= 3 && all(u %in% c(0, 1, 2))) {
    return(function(x) dplyr::case_when(
      x == 0 ~ "Down",
      x == 1 ~ "Neutral",
      x == 2 ~ "Up",
