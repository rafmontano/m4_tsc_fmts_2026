...
#
# Purpose:
#   Evaluate the Smyl and FFORMA baselines directly on real M4 series without
#   scaling or a train/test split.
# Inputs:
#   Global configuration and the x, xx, and pt_ff components of M4comp2018::M4.
# Outputs:
#   One consolidated evaluation RDS file per method and frequency.
# Run from:
...
if (USE_TEST_SUBSET) {
  M4_all <- readRDS(subset_clean_file)
} else {
...
for (period_i in periods) {
  ...
  process_one_series <- function(s) {
    ...
    if (length(xx_true) == 0L) {
      return(NULL)
    }
    if (any(!is.finite(xx_true))) {
      return(NULL)
    }
    if (is.null(s$pt_ff)) {
      return(NULL)
    ...
    if (any(!is.finite(xx_smyl))) {
      return(NULL)
    }
    if (any(!is.finite(xx_fforma))) {
      return(NULL)
    }
