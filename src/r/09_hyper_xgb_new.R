# =====================================================================
# 09_hyper_xgb_new.R
# Train one XGBoost model per horizon, by frequency and window mode
#
# Inputs (globals):
#   data_dir, models_dir, periods, FORCE_RERUN, SPLIT_SEED
#
# Requires:
#   - all_windows_with_features_<tag>_<window_tag>.rds
#   - split_index_<tag>_<window_tag>.rds
#
# Output:
#   models/xgb/<tag>_<window_tag>/model_h01.ubj
#   models/xgb/<tag>_<window_tag>/model_h02.ubj
#   ...
# =====================================================================
...
# ---------------------------------------------------------------------
# Helper: choose split for a given model_id
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Detect OS and return fixed nthread
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Helper: Bayesian optimization for binary XGBoost
# ---------------------------------------------------------------------
...
for (period_i in periods) {
  ...
  for (window_mode_i in window_modes) {
    ...
    for (h in seq_len(H_i)) {
      ...
      rm(y_all, train_df, test_df, y_train, y_test,
